...
namespace ImageProcess2
{
	...
	public class Coins
	{
		...
		public static void CountCoin(Bitmap bmp, ref Label countLabel, ref Label valueLabel, ref Label p5Count, ref Label p1Count, ref Label c25Count, ref Label c10Count, ref Label c5Count)
		{
			int count = 0;
			float value = 0;
			float curr_value = 0;
			int p5 = 0, p1 = 0, c25 = 0, c10 = 0, c5 = 0;
			...
			for (int i = 0; i < height; i++)
				for (int j = 0; j < width; j++)
				{
					...
					{
						count++;
						curr_value = Classify(BFS(bmp, ref visited, j, i), ref count);
						value += curr_value;
						if(curr_value == PESO_5)
						{
							p5++;
						} else if (curr_value == PESO_1)
                        {
							p1++;
						} else if (curr_value == CENT_25)
                        {
							c25++;
						} else if (curr_value == CENT_10)
                        {
							c10++;
						} else if(curr_value == CENT_5){ 
							c5++;
						}
					}
				}
            ...
            p5Count.Text = p5.ToString();
			p1Count.Text = p1.ToString();
			c25Count.Text = c25.ToString();
			c10Count.Text = c10.ToString();
			c5Count.Text = c5.ToString();
		}
